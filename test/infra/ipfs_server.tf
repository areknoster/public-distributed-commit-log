...
resource "google_compute_address" "external_ipfs_server_address" {
  name         = "${local.ipfs_server_name}-external-address"
...
resource "google_compute_instance" "ipfs_server_vm" {
  ...
  network_interface {
    ...
    subnetwork = google_compute_subnetwork.pdcl_test_subnetwork.self_link
    access_config {
      nat_ip = google_compute_address.external_ipfs_server_address.address
    }
  }
